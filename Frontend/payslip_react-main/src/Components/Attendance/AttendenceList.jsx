...
const AttendenceList = () => {
  ...
  const getUser = () => {
    axios.get("http://192.168.1.163:8092/attendance/all")
      .then((response) => {
  ...
  const getData = (employeeId) => {
    ...
    Navigate(`/attendanceManagement`, { state: { employeeId } })  //deleteuser/
  }
  ...
  const columns = [
    ...
    { 
      name: <h5><b>Mnth/Year</b></h5>,
      selector: (row) => `${row.month} ${row.year}`,
      width: "120px",
      wrap: true
    },
    {
      name: <h5><b>No.Of Working Days</b></h5>,
      selector: (row) => row.totalWorkingDays,

    },
    {
      name: <h5><b>No.Of Leaves</b></h5>,
